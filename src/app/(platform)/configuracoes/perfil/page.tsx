// src/app/(platform)/configuracoes/perfil/page.tsx
"use client";
    ...
    <div className="bg-card border rounded-lg shadow-sm p-6">
      ...
      <form action={profileFormAction} className="space-y-6">
        ...
        {profileState.message && (
          <div className={`p-4 rounded-lg border flex items-center gap-2 transition-all ${
        ...
        <div className="rounded-lg bg-blue-50 dark:bg-blue-900/20 p-4 border border-blue-200 dark:border-blue-800">
          ...
          <ul className="text-xs text-blue-800 dark:text-blue-200 space-y-1 ml-6">
            <li className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${currentPassword.length >= 8 ? 'bg-green-500' : 'bg-gray-400'}`} />
              Use pelo menos 8 caracteres
            </li>
            <li className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${/[A-Z]/.test(currentPassword) && /[a-z]/.test(currentPassword) ? 'bg-green-500' : 'bg-gray-400'}`} />
              Inclua letras maiúsculas e minúsculas
            </li>
            <li className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${/\d/.test(currentPassword) && /[^a-zA-Z\d]/.test(currentPassword) ? 'bg-green-500' : 'bg-gray-400'}`} />
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Tem certeza de que deseja alterar sua senha? Esta ação irá desconectar você de todos os dispositivos.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
...
export default function ProfilePage() {
  ...
  // Efeitos para mostrar toasts baseados no estado dos formulários
  useEffect(() => {
    if (profileState.success) {
      toast.success(profileState.message || 'Perfil atualizado com sucesso!');
  ...
  useEffect(() => {
    if (passwordState.success) {
      toast.success(passwordState.message || 'Senha alterada com sucesso!');
      if (passwordFormRef.current) {
